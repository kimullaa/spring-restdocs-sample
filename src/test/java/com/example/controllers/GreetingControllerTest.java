...
public class GreetingControllerTest {
    ...
    public void setUp() {
        this.mockMvc = MockMvcBuilders
                ...
                .alwaysDo(document("greeting/{method-name}",
                        requestParameters(
                                parameterWithName("hour").description("時刻: 必須入力(0-24)")
                        )))
                .build();
    }
    ...
    public void GoodMorning下限_正常系() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "5"))
    ...
    public void GoodMorning上限_正常系() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "15"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.greeting").value("good morning"));
    }
    ...
    public void GoodNight下限1_正常系() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "0"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
    ...
    public void GoodNight下限2_正常系() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "16"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.greeting").value("good night"));
    }
    ...
    public void GoodNight上限1_正常系() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "4"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$.greeting").value("good night"));
    }
    ...
    public void doc() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "24"))
                ...
                .andDo(document("greeting/{method-name}",
                        responseFields(
                                fieldWithPath("greeting").type(JsonFieldType.STRING).description("hourが0-4,16-24の場合はgood night,5-15の場合はgood morning")
                        )));
    }
    ...
    public void 入力値範囲外_上限超え() throws Exception {
        this.mockMvc.perform(get("/greeting").param("hour", "25"))
                .andExpect(status().isBadRequest())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8));
    }
